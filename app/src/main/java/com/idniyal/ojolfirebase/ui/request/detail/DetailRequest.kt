...
import com.idniyal.ojolfirebase.utils.Constan.key
...
class DetailRequest : AppCompatActivity(), OnMapReadyCallback {
    ...
    override fun onMapReady(googleMap: GoogleMap) {
        ...
        val latlng = booking?.latAwal?.let{ booking?.lonAwal?.let{
                it1 -> LatLng(it, it1) } }

        val latlng1 = booking?.latTujuan?.let{ booking?.lonTujuan?.let{
                it1 -> LatLng(it, it1) } }

        val res = this.resources
        val marker1 = BitmapFactory.decodeResource(res, R.mipmap.ic_pin)
        val smallmarker = Bitmap.createScaledBitmap(marker1, 80,
            120, false)
        ...
        //menampilkan marker secara kesuluruhan
